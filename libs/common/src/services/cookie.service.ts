...
export class CookieService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.accessTokenTtl = !parsedAccess || isNaN(parsedAccess) ? 3600000 : parsedAccess * 1000;
    this.refreshTokenTtl = !parsedRefresh || isNaN(parsedRefresh) ? 604800000 : parsedRefresh * 1000;
  ...
  setAccessToken(res: Response, token: string, deviceId: string) {
    ...
    const maxAge =
      typeof this.accessTokenTtl === 'number' && !isNaN(this.accessTokenTtl) ? this.accessTokenTtl : 3600000;
