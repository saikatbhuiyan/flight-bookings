...
import { IPaymentGateway, PaymentMethod, PAYMENT_METHOD_GATEWAY_MAP } from './payment-gateway.interface';
...
export class PaymentGatewayFactory {
  ...
  constructor(
    ...
    this.logger.log(`Gateway registry initialised with processors: [${[...this.registry.keys()].join(', ')}]`);
  }
  ...
  getByName(processorName: string): IPaymentGateway {
    ...
    if (!gateway) {
      ...
      throw new Error(`Unknown payment processor: "${processorName}". Available: [${available}]`);
    }
    return gateway;
  }
  ...
  resolve(paymentMethod?: PaymentMethod, gatewayOverride?: string): IPaymentGateway {
    if (gatewayOverride) {
      this.logger.warn(
        `Using explicit gateway override: "${gatewayOverride}". ` + `Prefer paymentMethod for automatic routing.`,
      );
  ...
  private register(gateway: IPaymentGateway): void {
    ...
    this.logger.log(
      `Registered processor "${name}" supporting methods: ` + `[${gateway.getSupportedPaymentMethods().join(', ')}]`,
    );
  }
  ...
  private getDefaultProcessorName(): string {
    ...
    if (!this.registry.has(configured)) {
      this.logger.warn(`PAYMENT_DEFAULT_PROCESSOR="${configured}" is not registered. ` + `Falling back to "stripe".`);
      return 'stripe';
    }
    return configured;
  }
