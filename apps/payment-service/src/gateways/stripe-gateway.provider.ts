...
export class StripeGatewayProvider implements IPaymentGateway {
  ...
  async createPaymentIntent(params: CreatePaymentIntentParams): Promise<PaymentIntent> {
    try {
      this.logger.log(
        `Creating Stripe PaymentIntent for booking ${params.bookingId} ` + `via method: ${params.paymentMethod}`,
      );
      ...
      this.logger.error(`Failed to create Stripe PaymentIntent: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async capturePayment(gatewayPaymentId: string): Promise<PaymentResult> {
    try {
      ...
      this.logger.error(`Failed to capture Stripe payment: ${error.message}`, error.stack);
      return {
        success: false,
