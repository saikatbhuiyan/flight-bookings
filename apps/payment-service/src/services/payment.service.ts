...
export class PaymentService {
  ...
  async createPaymentIntent(dto: CreatePaymentIntentDto): Promise<PaymentIntent> {
    this.logger.log(
      `Creating payment intent for booking ${dto.bookingId} ` + `[method: ${dto.paymentMethod ?? 'default'}]`,
    );
  ...
  async capturePayment(gatewayPaymentId: string): Promise<PaymentTransaction> {
    ...
    try {
      ...
      intent.status = result.success ? PaymentIntentStatus.SUCCEEDED : PaymentIntentStatus.FAILED;
      ...
      this.logger.log(`Payment ${result.success ? 'succeeded' : 'failed'}: ${savedTransaction.id}`);
