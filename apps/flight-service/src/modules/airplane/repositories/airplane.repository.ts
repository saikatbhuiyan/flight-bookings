...
export class AirplaneRepository implements IBaseRepository<Airplane> {
  ...
  async search(queryDto: QueryAirplaneDto): Promise<[Airplane[], number]> {
    ...
    if (manufacturer) {
      query.andWhere('airplane.manufacturer ILIKE :manufacturer', {
        manufacturer: `%${manufacturer}%`,
      });
    }

    if (modelNumber) {
      query.andWhere('airplane.modelNumber ILIKE :modelNumber', {
        modelNumber: `%${modelNumber}%`,
      });
    }

    if (registrationNumber) {
      query.andWhere('airplane.registrationNumber ILIKE :registrationNumber', {
