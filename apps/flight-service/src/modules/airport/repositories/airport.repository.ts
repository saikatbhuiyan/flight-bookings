...
export class AirportRepository implements IBaseRepository<Airport> {
    ...
    async search(queryDto: QueryAirportDto): Promise<[Airport[], number]> {
        const { search, code, icaoCode, cityId, cityName, country, active, sortBy, sortOrder, page = 1, limit = 10 } = queryDto;
        ...
        if (icaoCode) {
            query.andWhere('airport.icaoCode = :icaoCode', { icaoCode: icaoCode.toUpperCase() });
        }
        ...
        if (cityName) {
            query.andWhere('city.name ILIKE :cityName', { cityName: `%${cityName}%` });
        }

        if (country) {
            query.andWhere('city.country ILIKE :country', { country: `%${country}%` });
        }
