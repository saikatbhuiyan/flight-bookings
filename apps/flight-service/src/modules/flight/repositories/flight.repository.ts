...
export class FlightRepository implements IBaseRepository<Flight, number> {
    ...
    async search(searchDto: SharedSearchFlightDto): Promise<Flight[]> {
        ...
        if (departureAirport) {
            query.andWhere('departure.code = :dep', { dep: departureAirport.toUpperCase() });
        }

        if (arrivalAirport) {
            query.andWhere('arrival.code = :arr', { arr: arrivalAirport.toUpperCase() });
        }
    ...
    async delete(id: number): Promise<boolean> {
        const result = await this.repository.update(id, { status: 'CANCELLED' as any });
