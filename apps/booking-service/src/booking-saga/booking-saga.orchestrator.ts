...
export class BookingSagaOrchestrator {
    ...
    private async lockSeatsStep(state: SagaState): Promise<void> {
        ...
        this.logger.log(`Step 2: Locked ${state.data.seatNumbers.length} seats in Redis`);
    }
    ...
    private async confirmFlightSeatsStep(state: SagaState, booking: Booking): Promise<void> {
        ...
        try {
            ...
            switch (lastStep) {
                ...
                case SagaStep.SEATS_LOCKED:
                    // Release Redis locks
                    await this.seatLockService.releaseSeats(booking.flightId, state.bookingId);
                // Fall through
    ...
    private async releaseFlightSeatsStep(booking: Booking): Promise<void> {
